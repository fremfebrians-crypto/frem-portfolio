...
export async function PUT(
  ...
  try {
    ...
    const rawType = String(form.get('type') || existing.type).trim();
    const type = ['image', 'excel', 'pdf'].includes(rawType) ? rawType : 'image';
    ...
    if (type === 'pdf') {
      if (pdf instanceof File && pdf.size > 0) {
        const savedPdf = await uploadService.uploadPdf(pdf);
        updatedData.pdfPath = savedPdf;
      } else {
        updatedData.pdfPath = existing.pdfPath;
      }
